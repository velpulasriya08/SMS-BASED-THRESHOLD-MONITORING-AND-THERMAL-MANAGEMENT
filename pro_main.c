...
u8 cur_temp,m_no[]="9398919072",syntax[]="1234",r_temp,rm_no[11],j;

void extract_msg(u8 *p)
{
	u8 ch,k,t[]="current TEMP is:";
	...
	if(strstr((const char *)p,(const char *)syntax))
	{
		...
		switch(ch)
		{
			...
			case 'M':p=p+4+1;
					...
					Write_CMD_LCD(0x01);
					break;	
			case 'I':for(j=0;j<strlen((const char*)t);j++)
					{
						p[j]=t[j];
					}
					//j=j+1;
				...
				/*	while(k)
					...
					p[j--]='\0';*/
					send_sms_info((char *)m_no,(char *)p,k);
					break;
		}
	}
	else
